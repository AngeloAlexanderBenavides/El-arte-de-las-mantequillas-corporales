...
import './BuyButton.css'; // Mantenemos estilos extra si hacen falta, pero migraremos mucho a Tailwind
...
export default function BuyButton({ label, size = 'lg', id = 'btn-buy', variant = 'primary' }) {
  ...
  return (
    <SfButton
      ...
      size={size}
      className={`font-bold text-white shadow-md transition-all active:scale-95 ${bgClass} w-full max-w-lg mx-auto text-center py-4 text-xl rounded-full`}
    >
      {text}
    </SfButton>
  );
